...
let
	...
	// Objeto de datos para enviar a PHP
	const data = {
		score: scoreCount,
	  ...
	  // Enviar los valores a PHP mediante una solicitud fetch
	  fetch('js/score.php', {
		method: 'POST',
		headers: {
		...
		.then(data => {
		  // Manejar la respuesta de PHP aquí si es necesario
		  console.log(data);
		})
		.catch(error => {
...
// score
const databaseName = 'snake_online';
...
function sendScore() {
    ...
    .then((response) => {
        if (response.status === 200) {
            ...
            console.error('Error in sendScore:', response.statusText);
        }
    })
    .catch((error) => {
...
function gameLoop() {
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height); // Limpieza del canvas
    drawFood();
    drawSnake();
    if (diff === 'Hard') {
        ctx.strokeStyle = '#f00';
...
function withoutBorder() { // A function that checks if the snake is touched border
	if ( snake.x < 0 ) {
		...
		score();
		drawScore();
		    // Mostrar la alerta después de actualizar la puntuación
			alert("¡Perdiste! Puntuación final: " + lastScore);
			sendScore();
			const phpFile = 'js/score.php';
			const url = `${phpFile}?score=${scoreCount}&nombreUsuario=${nombreUsuario}&idUsuario=${idUsuario}`;
...
function restart() {
    // Agregar comprobación y actualización de la mejor puntuación
    bestScore();

    lastScore = scoreCount; // Guardar el puntaje antes de reiniciar
    drawScore();
    ...
    snake.body.forEach((e, index) => {
        ...
        for (let i = index + 1; i < snake.body.length; i++) {
            if (e.x === snake.body[i].x && e.y === snake.body[i].y) {
                audioPlay('dead');
                score(); // Asegúrate de llamar a score antes de reiniciar
                restart();
				drawScore();
		    // Mostrar la alerta después de actualizar la puntuación
			alert("¡Perdiste! Puntuación final: " + lastScore);
